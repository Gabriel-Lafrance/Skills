...
2. For `initial` and `full-rescan`, it provides the fixed-point diff, relevant spec, Active Rules, and current slice to separate Standards and Spec work, then requires adversarial Wave 2. Standards must Read `/taste` and `/architecture` this turn, run `code-review:blocker-vs-follow-up`, `code-review:naming-alignment`, and return the Wave 1 fences. Wave 2 must return the hunt re-inspect (not a cloned miss-class table).
