...
`analyzer`, `implementer`, `designer`, `reviewer`, `tester`)
...
a local diff or an open GitHub PR. `tester` writes a behavior lock when the user started `/create-test` or
...
They are not interchangeable. Do not use `implementer` for
screens and visible copy. There is no architect worker. [code-structure.md](../rules/code-structure.md) is a
...
## Read first
- `rules/code-quality.md` and `rules/code-structure.md` (hard, [standards.md](standards.md))
...
| Local diff vs the what and the parent task, or an open GitHub PR | Pack `reviewer`, or a harness general worker when no pack role exists |
...
| Standards and Spec review | Parallel specialists (plus extra passes when the diff has independent surfaces). See `/review`. No Design axis. No second adversarial wave |
...
4. If ready slices remain, update their status in **Current slices** and
   ...
   it then runs `/review` when the parent requires it.
...
- Spawning an architect worker ([code-structure.md](../rules/code-structure.md) is a bar, not a specialist)
...
- Worker running acceptance gates or `/review`
