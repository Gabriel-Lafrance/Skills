...
1. If the working tree has staged, unstaged, or untracked changes in scope, create a checkpoint commit on the typed branch, e.g. `wip: just-do-it checkpoint before CR1` (or `before CR1-remediation-N` / `before CR2`). Do not run the CI mirror for a checkpoint that will not be pushed. If a PR is already open, run the mirror in [pr-ship.md](../pack-shared/pr-ship.md) before that commit.
