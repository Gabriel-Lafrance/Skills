...
description: >-
  ...
  findings under .agents/temp/analyses/<id>/, optional user-helped sharpening,
...
**Standalone only.** Never call from `/goal`, never ship an `analyze-flow`. Does **not** write Linear/GitHub — that is `/write-ticket`.

Crunch the repo against an ask and **always persist findings to disk**. Optionally sharpen with the user. Optionally **promote** the analysis into a `/goal` workspace. Default end state is the analysis file — not a goal, not a ticket.

**Ask style:** [../asking.md](../asking.md). **Taste / structure:** `/taste-flow`, `/architecture` when relevant.
...
4. Do **not** treat this as `/grill-me-flow` goal gates unless promoting next
...
6. Announce both paths. If **e**, continue with `/goal` / `/grill-me-flow` as needed
