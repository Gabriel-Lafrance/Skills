...
2. For `initial` and `full-rescan`, it provides the fixed-point diff, relevant spec, Active Rules, and current slice to separate Standards and Spec work, then requires adversarial Wave 2. Standards must Read `/taste` and `/architecture` this turn, run the Named principles checklist, the **Naming alignment pass**, and return the **Principles sweep** and **Architecture sweep** tables (including Honest names).
