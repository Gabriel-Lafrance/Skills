...
- App UI follows `/design` (`design:experience`, `design:first-glance`, `design:no-obvious`, `design:professional-craft`, `design:ui-copy`, `design:quality-floor`): match the product's identity, do the next input when it is obvious, show only what everyone needs at first glance, do not caption an empty list, write copy that fits the surface (landing sells, docs explain, app acts), and ship finished, not a restyle draft
