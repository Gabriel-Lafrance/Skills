...
Copy the frontier and dependencies into **Current slices** of the execution context. Expand implementation-ready slices with an [inline plan contract](../task/reference.md#inline-plan-contract), then the designer for user-facing frontier work and `/implement` for non-UI. Do not create an INDEX or rely on an automatic artifact.
