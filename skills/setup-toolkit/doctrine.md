...
Put lint, format, and principle quality gates **in the app repo**, and copy this pack’s rules into the user rules folder Cursor reads on this machine. Cursor plugins cannot run ESLint or Prettier for a project that has no config, and plugin rules are not a reliable apply path. This skill copies this pack’s templates and rule files.

## Owns

Which templates to copy, what not to overwrite, copying `rules/*.mdc` into `~/.cursor/rules/gabriel-skills/`, Convex plugin detection, the quality-gate tests (`test:quality`), the mutant check (`test:mutants`), the smoke check, and whether to start `/design` Initialization when `docs/design.md` is missing.

## Does not own

- Rewriting an existing lint stack
- Reformatting the repo as part of setup
- Pasting rule text into the Customize rules text box
- Copying rules into the app repo unless the user asks to pin them
...
1. **User rules first.** Copy `<pack-root>/rules/*.mdc` into `~/.cursor/rules/gabriel-skills/` before lint setup. This step does not need `package.json`. Overwrite only that folder’s pack files. Do not touch other files under `~/.cursor/rules`. Do not paste a second copy into Customize → Rules.
2. **Fail fast on lint setup** if there is no `package.json` at the workspace root (or the obvious app root the user named). ESLint and Prettier belong in JS/TS apps, not in this markdown pack itself. The user-rules copy still stands.
...
- `~/.cursor/rules/gabriel-skills/*.mdc` copied from `<pack-root>/rules/` (Windows: `%USERPROFILE%\.cursor\rules\gabriel-skills\`)
...
Copy pack rules into the user rules folder first ([reference.md](reference.md#install-user-rules)). Then write configs next to the app `package.json`. That is the normal home for ESLint and Prettier. Do not invent `services/lint/` or a wrapper package unless this repo already publishes shareable configs that way.
...
- Skipping the user-rules copy
- Pasting the same rule text into the Customize rules text box
- Overwriting a user rule outside `~/.cursor/rules/gabriel-skills/`
- Copying rules into the app `.cursor/rules/` when the user did not ask
