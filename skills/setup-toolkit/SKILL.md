...
description: >-
  ...
  test, test:quality, or to add the engineering toolkit to an app. Bans
...
Add **ESLint**, **Prettier**, Cursor/VS Code workspace files, and **quality-gate tests** (`test:quality`: cyclomatic complexity (McCabe) plus principle gates) to the **current app repo**. Templates live in [templates/](templates/). Do not invent a different stack.

The ESLint templates include a no-emdash rule (em dash, en dash, horizontal bar) and a cyclomatic cap of 5. Workspace files recommend the ESLint and Prettier extensions. Gate failures use **plain (Classic)** — `keep this simple (KISS)`.
