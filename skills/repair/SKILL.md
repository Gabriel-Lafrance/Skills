...
2. **Grill before you cut** — when you find a defect, **`/grill-me`** on *what* to fix and *how* (smallest footprint options). Do not silently rewrite
...
5. **One bug, one cut** — no drive-by refactors
6. **Massive → escalate** — multi-feature / multi-layer / redesign → stop and send to `/goal`
...
Run **`/grill-me`** (one question at a time, recommended answer first). Cover at least:
...
Persist locks in `GRILL.md`. **Stop** for shared understanding yes before coding.
