...
- App UI follows [user-experience.md](../rules/user-experience.md) (`design:experience`, `design:first-glance`, `design:no-obvious`, `design:professional-craft`, `design:ui-copy`, `design:spoken-locale`, `design:quality-floor`): match the product's identity, do the next input when it is obvious, show only what everyone needs at first glance, do not caption an empty list, write copy that fits the surface (landing sells, docs explain, app acts), translate the job into words speakers actually use, and ship finished, not a restyle draft
