...
Completions; do not solo non-trivial labor or grep the tree. It takes
recommended soft decisions,
...
3. Run `/analyze` (explorers find, then analyzers judge; this parent owns the
   next step), then `/task` for the bounded build (`/task` returns evidence;
   this parent owns shipping).
