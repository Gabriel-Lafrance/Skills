...
- When you change pack-root `AGENTS.md`, copy the same file to `skills/setup-toolkit/templates/AGENTS.md` in that change. The two files stay byte-identical. Setup uses the root file when the pack is on disk, and the template when only the skill was installed. App repos get that full copy. Claude and Codex point at the installed template.
...
- `npx skills` does not install root `AGENTS.md`. The setup skill ships `templates/AGENTS.md` (keep it identical to the pack-root file). `/setup-toolkit` copies that contract into the app when the user chose the repo and the app file is missing or already the pack copy (marker `gabriel-skills-agents`). A different app `AGENTS.md` stays put. When they chose user data, Claude Code writes one `@` import of the installed `setup-toolkit/templates/AGENTS.md` in `~/.claude/CLAUDE.md` and does not write `~/.claude/gabriel-skills/AGENTS.md`. Codex symlinks `AGENTS.md` in the Codex home to that same template, and copies the template bytes only if the symlink cannot be created (and says so). Gemini CLI and Aider keep their existing rows. Cursor is the repo file. Setup does not write a Cursor `hooks.json`. If `gabriel-skills/follow-agents.mdc` is already on disk, setup deletes that file only. It does not create a harness directory the user does not have, and it does not add a project `CLAUDE.md`. ESLint and quality gates wait for a yes.
