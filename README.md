...
**[`/ask-gabriel`](./skills/ask-gabriel/SKILL.md)** is the front door — sole auto-invokable skill. Agents should use it often when unsure what to run. Dual skills (`/grill-me`, `/taste`, `/architecture`, …) pick standalone vs flow themselves ([variants.md](./skills/pack-shared/variants.md)).

| Skill | When to use |
| --- | --- |
| [`/ask-gabriel`](./skills/ask-gabriel/SKILL.md) | Unsure which skill; what next; before non-trivial work |
| [`/goal`](./skills/goal/SKILL.md) | Build a feature end-to-end with user-locked Active Rules (grill → plans → implement → validate → review). Always loads `/taste`; loads `/architecture` unless trivial. Pass a ticket: `/goal IN-1234` or `/goal #42`. |
...
Internals (`/validate`, `/implement`, `/orchestrate`, …) are looked up by `/goal` or `/repair` — not typical user entries.
