...
[subagents.md](../pack-shared/subagents.md), and [reference.md](reference.md).
...
- Lock type from Linear metadata where possible: Feature → `feature/`, Tweak →
  `tweak/`, Bug → `bug/`, Refactor → `refactor/`, Chore → `chore/`, Hotfix →
  `hotfix/`. Otherwise infer from the ticket and announce it Locked.
