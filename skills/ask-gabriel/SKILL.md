...
| ESLint / Prettier / lint, format, dead code, mutants, quality gate (`test:quality`, `test:mutants`), or pack rules not loading | `/setup-toolkit` (copies rules into `~/.cursor/rules/gabriel-skills/`; also starts `/design` Initialization if `docs/design.md` is missing) |
