...
2. Draft the **Structure** card from `architecture/doctrine.md` before proposing new files.
3. Prefer the smallest layout that meets the outcome. Name a required behavior-preserving move when the current goal or a named finding needs it; otherwise record it as a follow-up.
4. Return the Structure card and a short recommendation. Do not implement unless the parent asked you to.

## Must not

- Invent a parallel billing/auth/email path inside a feature
