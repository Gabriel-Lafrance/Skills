...
This skill is a user start. Do not nest it under `/goal`. Parent orchestrator
...
3. Run `/analyze` (this parent owns the next step), then `/goal` for the
   bounded build (`/goal` returns evidence; this parent owns shipping).
4. Checkpoint-commit the working tree; pin `baseSha...headSha`. Run
   `/code-review` (CR1); remediate Fix-now with `/analyze` + `/goal`
