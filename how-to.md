...
- **Unslop:** every assistant **reply in this discussion** follows [`/unslop`](./skills/unslop/SKILL.md) via [standards.md](./skills/pack-shared/standards.md) (Read the doctrine, draft the chat message clean). `/unslop` rewrites a prior reply when the user says the last message sounded generated. Do not auto-invoke it. Do not use it as README, ticket, or PR-body cleanup.
- **Standards:** every pack skill except `/ask-gabriel` links [`pack-shared/standards.md`](./skills/pack-shared/standards.md) and **Reads** `/taste` plus `/architecture` doctrines on every run, and `/unslop` before the reply in this discussion. `/ask-gabriel` stays thin and does not load the bodies (gold-standards still loads unslop for that reply).
...
3. Link `asking.md` if the skill asks the user anything. Link `standards.md` on every skill except `/ask-gabriel`. Link `plain-language.md` if the skill talks to the user. The chat reply also follows `/unslop` via standards.
4. Wire discovery:
