...
2. **Never overwrite** an existing ESLint or Prettier config, ignore file, or a script that already exists. Report what you skipped.
3. **One stack.** Templates in [templates/](templates/) only. Do not add extra ESLint plugins “for completeness.”
...
If the repo already has a working lint/format story, **stop after reporting it**. Offer `/setup-toolkit` only to fill missing pieces (for example Prettier when ESLint already exists).
