...
## Design
- **File:** `docs/design.md` present | missing (the designer writes a short Do / Don't list from the routes in code before UI work)
- **Worker:** designer (user-facing) | `/implement` (non-UI) | split
...
3. Unless the skip rule applies, run `/grill-me` fully. It applies the **Taste** and **Architecture** sections of `AGENTS.md` on every run ([standards.md](../pack-shared/standards.md)). For user-facing work it also pulls in the App UX section of `AGENTS.md` and the current `docs/design.md`.
